 ...
 * 一个进程可有多个队列。
 * 队列可分并发队列和串行队列。
 添加关谈iOS多线程(NSThread、NSOperation、GCD)编程
 文章配图
 ...
 * 队列可分并发队列和串行队列。
 ...
 显然动态创建线程多了几行代码，其实就是那几行代码，如果重复编写数遍那是一件多么不爽的事情。首次看来静态方法创作线程和隐式创建线程显得比较方便，简洁。从知识结构来说，讲到这里应该讲述一下线程锁，鉴于并不常用和文章过长就不在此详细讲述，有兴趣可以自行查阅。
